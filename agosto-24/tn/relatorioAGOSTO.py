...
portal_usuariosUnicos_2024Table = [1633738,1440822,1360093,1686858,1523523,1409476,1753667,1898844]
portal_usuariosRescorrentes_2024Table = [723672,551399,610932,713143,695163,628876,647637,704678]

portal_usuariosUnicos_2024Analytics = [1100000,970000,872000,1100000,985000,936000,1300000,1400000]
portal_usuariosRescorrentes_2024Analytics = [279000,248000,233000,267000,263000,244000,265000,306000]

portal_visualizacoes_2024 = [3503660,2645684,3109309,3569089,3274745,3006463,3524319,3505334] # mesmo valor na tabela e no analytics
portal_novosUsuarios_2024 = [819084,778346,726313,910001,812300,764468,1101211,1194579] # mesmo valor na tabela e no analytics
...
ig_seg_2024 = [6931,5816,5835,7843,6717,5573,5369,6072]
ig_seg_2024_perdeu = [5485,4945,4587,4885,4649,4326,4987,6082]
ig_alcance_2024 = [542064,658917,633648,847362,941640,930258,1008296,1158467]
ig_vivitas_2024 = [151885,144742,131296,162036,133497,117925,110346,135437]

ig_seg_2024_total = [529865,530797,532444,535373,537733,538756,539028,538982]
...
fb_seg_2024 = [628,389,188,295,254,251,265,516]
fb_seg_2024_perdeu = [162,162,176,171,158,132,160,213]
fb_alcance_2024 = [467889,390008,197628,292192,257199,254134,266380,675762]
fb_vivitas_2024 = [32152,31459,31499,31612,29294,22864,24050,26171]

fb_seg_2024_total = [332603,332614,332419,332336,332214,332133,332006,332122]
...
yb_inc_2024 = [415,614,1506,1235,536,439,684,1690]
yb_inc_2024_perdeu = [85,84,110,97,83,76,100,142]
yb_visualizacoes_2024 = [132377,137318,371375,339297,139698,94934,175389,431106]
yb_horas_2024 = [2339,4194,5135,5316,3288,2302,2724,5438]

yb_inc_2024_total = [34000,34500,35940,37092,37544,37922,38507,40032]
...
with doc.create(Enumerate(enumeration_symbol=r"•")) as itemize:     
    ...
    with itemize.create(Enumerate(enumeration_symbol=r"-")) as sublist:
        sublist.add_item(NoEscape(r"Em geral, \textbf{Agosto} foi o melhor mês até o momento. Esse foi o mês com o maior número de novos usuários, visualizações e usuários recorrentes,tendo o maior crescimento percentual em duas dessas métricas e o segundo maior em visualizações. \textbf{Abril e julho} vem logo em seguida e ambos também possuem bons crescimentos percentuais."))
...
with doc.create(Enumerate(enumeration_symbol=r"•")) as itemize:     
    itemize.add_item("Mês anterior:")
    with itemize.create(Enumerate(enumeration_symbol=r"-")) as sublist:
        sublist.add_item(NoEscape(r"\textbf{Agosto} foi o melhor mês até o momento. \textbf{Março} vem em seguida e ambos possuem bons crecimentos percentuais."))
...
doc.generate_pdf(fr'C:\Users\{GR.path_aliss}\Documents\Repositórios\Relatórios\TN\Relatório-TN_Agosto-2024', clean_tex=True)
