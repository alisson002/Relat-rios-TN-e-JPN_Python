...
portal_usuariosUnicos_2024Table = [1633738,1440822,1360093,1686858,1523523,1409476,1753667,1898844,1370851]
portal_usuariosRescorrentes_2024Table = [723672,551399,610932,713143,695163,628876,647637,704678,612918]

portal_usuariosUnicos_2024Analytics = [1100000,970000,872000,1100000,985000,936000,1300000,1400000,926000]
portal_usuariosRescorrentes_2024Analytics = [279000,248000,233000,267000,263000,244000,265000,306000,251000]

portal_visualizacoes_2024 = [3503660,2645684,3109309,3569089,3274745,3006463,3524319,3505334,2813497] # mesmo valor na tabela e no analytics
portal_novosUsuarios_2024 = [819084,778346,726313,910001,812300,764468,1101211,1194579,759312] # mesmo valor na tabela e no analytics
...
ig_seg_2024 = [6931,5816,5835,7843,6717,5573,5369,6072,5521]
ig_seg_2024_perdeu = [5485,4945,4587,4885,4649,4326,4987,6082,4603]
ig_alcance_2024 = [542064,658917,633648,847362,941640,930258,1008296,1158467,922259]
ig_vivitas_2024 = [151885,144742,131296,162036,133497,117925,110346,135437,109653]

ig_seg_2024_total = [529865,530797,532444,535373,537733,538756,539028,538982,539983]
...
fb_seg_2024 = [628,389,188,295,254,251,265,516,230]
fb_seg_2024_perdeu = [162,162,176,171,158,132,160,213,203]
fb_alcance_2024 = [467889,390008,197628,292192,257199,254134,266380,675762,205619]
fb_vivitas_2024 = [32152,31459,31499,31612,29294,22864,24050,26171,21062]

fb_seg_2024_total = [332603,332614,332419,332336,332214,332133,332006,332122,331921]
...
yb_inc_2024 = [415,614,1506,1235,536,439,684,1690,1724]
yb_inc_2024_perdeu = [85,84,110,97,83,76,100,142,172]
yb_visualizacoes_2024 = [132377,137318,371375,339297,139698,94934,175389,431106,657651]
yb_horas_2024 = [2339,4194,5135,5316,3288,2302,2724,5438,9389]

yb_inc_2024_total = [34000,34500,35940,37092,37544,37922,38507,40032,41578]
...
with doc.create(Enumerate(enumeration_symbol=r"•")) as itemize:     
    ...
    with itemize.create(Enumerate(enumeration_symbol=r"-")) as sublist:
        sublist.add_item(NoEscape(r"Em geral, \textbf{Agosto} foi o melhor mês até o momento. Esse foi o mês com o maior número de novos usuários e usuários recorrentes, com as visualizações bem próximas de \textbf{Abril e Julho} que também foram bons meses e possuem bons crescimentos percentuais."))
        ...
        sublist.add_item(NoEscape(r"\textbf{Abril} é o segundo melhor mês até o momento."))
...
with doc.create(Enumerate(enumeration_symbol=r"•")) as itemize:     
    ...
    with itemize.create(Enumerate(enumeration_symbol=r"-")) as sublist:
        ...
        sublist.add_item(NoEscape(r"\textbf{Janeiro} teve os segundos melhores números em novos seguidores(junto de \textbf{maio}) e visitas."))
        sublist.add_item(NoEscape(r"\textbf{Janeiro} teve os números mais baixos em alcance. \textbf{Junho} teve o menor número de visitas e novos seguidores."))
        sublist.add_item(NoEscape(r"\textbf{Setembro} teve bons números em alcance, mas não nas outras métricas."))
...
with doc.create(Enumerate(enumeration_symbol=r"•")) as itemize:     
    itemize.add_item("Mês anterior:")
    with itemize.create(Enumerate(enumeration_symbol=r"-")) as sublist:
        sublist.add_item(NoEscape(r"\textbf{Janeiro} foi o melhor mês em novos seguidores e visitas. Também teve o melhor crescimento em visitas."))
        ...
        sublist.add_item(NoEscape(r"\textbf{Setembro} está entre os meses com os menores números em todas as métricas."))
...
with doc.create(Enumerate(enumeration_symbol=r"•")) as itemize:     
    ...
    with itemize.create(Enumerate(enumeration_symbol=r"-")) as sublist:
        ...
        sublist.add_item(NoEscape(r"De \textbf{abril} a \textbf{junho} as métricas pararam de crescer. A partir de \textbf{julho} as métricas continuaram crescendo até o momento."))
...
doc.generate_pdf(fr'C:\Users\{GR.path_aliss}\Documents\Repositórios\Relatórios\TN\Relatório-TN_SET-2024', clean_tex=True)
