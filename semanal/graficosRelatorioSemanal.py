...
def visualizacoesIG(total_visualizacoes, anterior, porcentagem_maior, porcentagem_menor):
    ...
    wedges, texts, autotexts = ax.pie(
        sizes, labels=None, autopct="%.1f%%", startangle=90, colors=cores, wedgeprops={"width": 0.3, "edgecolor": "white", "linewidth": 2}
    )

    for autotext in autotexts:
        autotext.set_text('')

    # Adiciona o total de visualizações no centro
    ax.text(0, 1.2, f'{formataNumero(total_visualizacoes)}\nVisualizações', ha='center', va='center', fontsize=14, fontweight='bold')
    ax.text(0, 0, f'{crescimento(total_visualizacoes,anterior)}', ha='center', va='center', fontsize=14, fontweight='bold')
    
    # Adiciona as legendas do lado esquerdo e direito
    ax.annotate(
        f"{porcentagem_maior}%\nSeguidores",
        xy=(-1.05, 0), xycoords='data',
        textcoords='data', ha='right', fontsize=12, color=cores[0], fontweight='bold'
    )
    ax.annotate(
        f"{porcentagem_menor}%\nNão seguidores",
        xy=(1.05, 0), xycoords='data',
        textcoords='data', ha='left', fontsize=12, color=cores[1], fontweight='bold'
    )
    
    visualizacoesIG_plot_path = fr"C:/Users/{path_Usuarios}/Documents/Repositórios/Imagens/TN/visualizacoesIG.png"
    plt.savefig(visualizacoesIG_plot_path, bbox_inches="tight")

    return visualizacoesIG_plot_path

def grafico_barras_composto(porcentagens):
    """
    Gera um gráfico de barras composto horizontal com as categorias 
    Publicações, Reels, Stories e Vídeos, na ordem inversa, com barras arredondadas.
    
    :param porcentagens: Lista de 12 valores representando as porcentagens:
                         - Para cada categoria (publicações, reels, stories, vídeos):
                           Seguidores, Não seguidores e Contribuição total.
    """
    ...
    if len(porcentagens) != 12:
        raise ValueError("A lista de porcentagens deve conter exatamente 12 valores.")
    
    # Converte porcentagens para uma matriz 4x3 (cada linha: seguidores, não seguidores, contribuição total)
    porcentagens = np.array(porcentagens).reshape(4, 3)[::-1]
    
    # Calcula a parte "vazia" de cada barra (complemento para 100%)
    complementos = 100 - porcentagens[:, 0:2].sum(axis=1)
    complementos[complementos < 0] = 0  # Não permite valores negativos
    
    # Dados para plotagem
    base = np.zeros(4)  # Base inicial para empilhamento
    fig, ax = plt.subplots(figsize=(10, 6))
    
    # Cria as barras empilhadas horizontalmente
    for i in range(2):  # Dois segmentos por barra (Seguidores e Não Seguidores)
        ax.barh(
            categorias,
            porcentagens[:, i],
            left=base,
            color=cores[i],
            height=0.4,  # Torna as barras mais finas
            edgecolor='white',  # Adiciona bordas pretas
            linewidth=0.5,  # Define a espessura das bordas
            label='Seguidores' if i == 0 else 'Não seguidores',
            align='center'
        )
        base += porcentagens[:, i]  # Atualiza a base para o próximo segmento
    
    # Adiciona o "restante" como espaço vazio (se necessário)
    ax.barh(
        categorias,
        complementos,
        left=base,
        color='lightgray',
        height=0.4,  # Mantém a espessura consistente
        edgecolor='white',  # Adiciona bordas pretas
        linewidth=0.5,
        align='center'
    )
    ...
    # Adiciona os valores das porcentagens diretamente nas barras
    for i, cat in enumerate(categorias):
        total = 0
        for j in range(2):  # Itera sobre seguidores e não seguidores
            if porcentagens[i, j] > 0:
                ax.text(
                    total + porcentagens[i, j] / 2, i, f'{porcentagens[i, j]:.1f}%',
                    ha='center', va='center', color='white', fontsize=10
                )
                total += porcentagens[i, j]
        # Adiciona o valor de contribuição total no final da barra
        ax.text(
            102, i, f'{porcentagens[i, 2]:.1f}%',
            ha='left', va='center', color='black', fontsize=10, fontweight='bold'
        )
    
    # Adiciona título e ajusta layout
    ax.set_title('Visualizações por Tipo de Conteúdo')
    plt.tight_layout()
    grafico_barras_composto_plot_path = fr"C:/Users/{path_Usuarios}/Documents/Repositórios/Imagens/TN/grafico_barras_composto.png"
    plt.savefig(grafico_barras_composto_plot_path, bbox_inches="tight")

    return grafico_barras_composto_plot_path

def seguidorIG():
    # Define o caminho para o arquivo
    path_aliss = "aliss"  # Substitua pelo seu usuário
    ...
    # Verifica se a coluna 'Primary' existe no arquivo
    if 'Primary' not in seguidoresIG.columns:
        raise ValueError("A coluna 'Primary' não foi encontrada no arquivo. Verifique o arquivo de entrada.")

    # Ordena os dados por data
    seguidoresIG = seguidoresIG.sort_values('Data')

    # Divide o DataFrame em duas semanas (7 dias cada, consecutivos)
    seguidoresIG_ANTERIOR = seguidoresIG.iloc[:7]  # Primeiros 7 dias
    seguidoresIG_ATUAL = seguidoresIG.iloc[7:14]  # Próximos 7 dias

    # Verifica se ambos os DataFrames têm exatamente 7 registros
    while len(seguidoresIG_ANTERIOR) < 7:
        seguidoresIG_ANTERIOR = pd.concat([pd.DataFrame([{'Data': None, 'Primary': 0}]), seguidoresIG_ANTERIOR])
    while len(seguidoresIG_ATUAL) < 7:
        seguidoresIG_ATUAL = pd.concat([seguidoresIG_ATUAL, pd.DataFrame([{'Data': None, 'Primary': 0}])])

    # Alinha os dados
    seguidoresIG_ANTERIOR = seguidoresIG_ANTERIOR.reset_index(drop=True)
    seguidoresIG_ATUAL = seguidoresIG_ATUAL.reset_index(drop=True)

    # Dados para o gráfico
    datas_anteriores = seguidoresIG_ANTERIOR['Data'].dt.strftime('%d/%m').fillna('N/A').values  # Apenas dia/mês ou 'N/A'
    datas_recente = seguidoresIG_ATUAL['Data'].dt.strftime('%d/%m').fillna('N/A').values  # Apenas dia/mês ou 'N/A'
    valores_anteriores = seguidoresIG_ANTERIOR['Primary'].values
    valores_recente = seguidoresIG_ATUAL['Primary'].values

    # Configuração do gráfico
    x = np.arange(7)  # Posição das barras para 7 dias
    ...
    # Adiciona as datas coloridas abaixo de cada barra
...
def visitaIG():
    ...
    # Verifica se a coluna 'Primary' existe no arquivo
    if 'Primary' not in visitasIG.columns:
        raise ValueError("A coluna 'Primary' não foi encontrada no arquivo. Verifique o arquivo de entrada.")

    # Divide o DataFrame em duas semanas: anteriores (primeiros 7 dias) e atuais (últimos 7 dias)
    visitasIG = visitasIG.sort_values('Data')
    visitasIG_ANTERIOR = visitasIG.iloc[:7]
    visitasIG_ATUAL = visitasIG.iloc[-7:]

    # Garante que ambos os conjuntos de dados têm exatamente 7 registros
    while len(visitasIG_ANTERIOR) < 7:
        visitasIG_ANTERIOR = pd.concat([pd.DataFrame([{'Data': None, 'Primary': 0}]), visitasIG_ANTERIOR])
    while len(visitasIG_ATUAL) < 7:
        visitasIG_ATUAL = pd.concat([visitasIG_ATUAL, pd.DataFrame([{'Data': None, 'Primary': 0}])])

    # Alinha os dados
    visitasIG_ANTERIOR = visitasIG_ANTERIOR.reset_index(drop=True)
    visitasIG_ATUAL = visitasIG_ATUAL.reset_index(drop=True)

    # Dados para o gráfico
    datas_anteriores = visitasIG_ANTERIOR['Data'].dt.strftime('%d/%m').fillna('N/A').values  # Apenas dia/mês ou 'N/A'
    datas_recente = visitasIG_ATUAL['Data'].dt.strftime('%d/%m').fillna('N/A').values  # Apenas dia/mês ou 'N/A'
    valores_anteriores = visitasIG_ANTERIOR['Primary'].values
    valores_recente = visitasIG_ATUAL['Primary'].values
...
def alcanceeIG():
    ...
    # Verifica se a coluna 'Primary' existe no arquivo
    if 'Primary' not in alcanceIG.columns:
        raise ValueError("A coluna 'Primary' não foi encontrada no arquivo. Verifique o arquivo de entrada.")

    # Ordena os dados por data
    alcanceIG = alcanceIG.sort_values('Data')

    # Divide o DataFrame em duas semanas (7 dias cada, consecutivos)
    alcanceIG_ANTERIOR = alcanceIG.iloc[:7]  # Primeiros 7 dias
    alcanceIG_ATUAL = alcanceIG.iloc[7:14]  # Próximos 7 dias

    # Garante que ambos os conjuntos de dados têm exatamente 7 registros
    while len(alcanceIG_ANTERIOR) < 7:
        alcanceIG_ANTERIOR = pd.concat([pd.DataFrame([{'Data': None, 'Primary': 0}]), alcanceIG_ANTERIOR])
    while len(alcanceIG_ATUAL) < 7:
        alcanceIG_ATUAL = pd.concat([alcanceIG_ATUAL, pd.DataFrame([{'Data': None, 'Primary': 0}])])

    # Alinha os dados
    alcanceIG_ANTERIOR = alcanceIG_ANTERIOR.reset_index(drop=True)
    alcanceIG_ATUAL = alcanceIG_ATUAL.reset_index(drop=True)

    # Dados para o gráfico
    datas_anteriores = alcanceIG_ANTERIOR['Data'].dt.strftime('%d/%m').fillna('N/A').values  # Apenas dia/mês ou 'N/A'
    datas_recente = alcanceIG_ATUAL['Data'].dt.strftime('%d/%m').fillna('N/A').values  # Apenas dia/mês ou 'N/A'
    valores_anteriores = alcanceIG_ANTERIOR['Primary'].values
    valores_recente = alcanceIG_ATUAL['Primary'].values
    ...
    return alcanceIG_plot_path

def visualizacoeesIG():
    # Define o caminho para o arquivo
    path_aliss = "aliss"  # Substitua pelo seu usuário
    ...
    # Verifica se a coluna 'Primary' existe no arquivo
    if 'Primary' not in visualizacoesIG.columns:
        raise ValueError("A coluna 'Primary' não foi encontrada no arquivo. Verifique o arquivo de entrada.")

    # Ordena os dados por data
    visualizacoesIG = visualizacoesIG.sort_values('Data')

    # Divide o DataFrame em duas semanas (7 dias cada, consecutivos)
    visualizacoesIG_ANTERIOR = visualizacoesIG.iloc[:7]  # Primeiros 7 dias
    visualizacoesIG_ATUAL = visualizacoesIG.iloc[7:14]  # Próximos 7 dias

    # Garante que ambos os conjuntos de dados têm exatamente 7 registros
    while len(visualizacoesIG_ANTERIOR) < 7:
        visualizacoesIG_ANTERIOR = pd.concat([pd.DataFrame([{'Data': None, 'Primary': 0}]), visualizacoesIG_ANTERIOR])
    while len(visualizacoesIG_ATUAL) < 7:
        visualizacoesIG_ATUAL = pd.concat([visualizacoesIG_ATUAL, pd.DataFrame([{'Data': None, 'Primary': 0}])])

    # Alinha os dados
    visualizacoesIG_ANTERIOR = visualizacoesIG_ANTERIOR.reset_index(drop=True)
    visualizacoesIG_ATUAL = visualizacoesIG_ATUAL.reset_index(drop=True)

    # Dados para o gráfico
    datas_anteriores = visualizacoesIG_ANTERIOR['Data'].dt.strftime('%d/%m').fillna('N/A').values  # Apenas dia/mês ou 'N/A'
    datas_recente = visualizacoesIG_ATUAL['Data'].dt.strftime('%d/%m').fillna('N/A').values  # Apenas dia/mês ou 'N/A'
    valores_anteriores = visualizacoesIG_ANTERIOR['Primary'].values
    valores_recente = visualizacoesIG_ATUAL['Primary'].values
    ...
    plt.tight_layout()
    visualizacoes_plot_path = fr"C:/Users/{path_Usuarios}/Documents/Repositórios/Imagens/TN/visualizacoes.png"
    plt.savefig(visualizacoes_plot_path, bbox_inches="tight")
    
    return visualizacoes_plot_path

def grafico_cidades(cidades, porcentagens):
    """
    Gera um gráfico de barras simples para cidades e suas porcentagens, com as barras invertidas.

    :param cidades: Lista com os nomes das cidades.
    :param porcentagens: Lista com as porcentagens que representam cada cidade.
    """
    if len(cidades) != len(porcentagens):
        raise ValueError("A lista de cidades e a lista de porcentagens devem ter o mesmo tamanho.")

    # Inverte a ordem das cidades e porcentagens
    cidades = cidades[::-1]
    ...
    ax.set_yticks(y)
    ax.set_yticklabels(cidades)
    ax.set_xlim(0, max(porcentagens) + 3)
    ax.set_title('Proporção de Seguidores por Cidade (RN)', fontsize=14)
    ax.set_xlabel('')
    ax.set_ylabel('')
    ...
    cidadesIG_plot_path = fr"C:/Users/{path_Usuarios}/Documents/Repositórios/Imagens/TN/cidadesIG.png"
    plt.savefig(cidadesIG_plot_path, bbox_inches="tight")
    
    return cidadesIG_plot_path

def grafico_faixa_etaria(faixas, porcentagens):
    """
    Gera um gráfico de barras simples para faixas etárias e suas porcentagens.

    :param faixas: Lista com os nomes das faixas etárias.
    :param porcentagens: Lista com as porcentagens que representam cada faixa etária.
    """
    if len(faixas) != len(porcentagens):
        raise ValueError("A lista de faixas etárias e a lista de porcentagens devem ter o mesmo tamanho.")
    ...
    ax.set_ylim(0, max(porcentagens) + 10)
    ax.set_title('Proporção de Seguidores por Faixa Etária', fontsize=14)
    ax.set_xlabel('')
    ax.set_ylabel('')
    ...
    ax.spines['bottom'].set_visible(False)
    # ax.tick_params(left=False, bottom=False, labelbottom=False)
    ax.tick_params(right=False, bottom=False, labelbottom=True, labelleft=False)

    FEIG_plot_path = fr"C:/Users/{path_Usuarios}/Documents/Repositórios/Imagens/TN/FEIG.png"
    plt.savefig(FEIG_plot_path, bbox_inches="tight")
    
    return FEIG_plot_path

def sexoIG(total_visualizacoes, anterior, porcentagem_maior, porcentagem_menor):
    ...
    # Adiciona o total de visualizações no centro
    ax.text(0, 1.2, f'{formataNumero(total_visualizacoes)}\Seguidores', ha='center', va='center', fontsize=14, fontweight='bold')
    ax.text(0, 0, f'{crescimento(total_visualizacoes,anterior)}', ha='center', va='center', fontsize=14, fontweight='bold')
    
    # Adiciona as legendas do lado esquerdo e direito
    ax.annotate(
        f"{porcentagem_maior}%\Mulheres",
        xy=(-1.05, 0), xycoords='data',
        textcoords='data', ha='right', fontsize=12, color=cores[0], fontweight='bold'
    )
    ax.annotate(
    ...
    sexoIG_plot_path = fr"C:/Users/{path_Usuarios}/Documents/Repositórios/Imagens/TN/sexoIG.png"
    plt.savefig(sexoIG_plot_path, bbox_inches="tight")

    return sexoIG_plot_path

def interacoesIG(total_visualizacoes, anterior, porcentagem_maior, porcentagem_menor):
    ...
    # Adiciona o total de visualizações no centro
    ax.text(0, 1.2, f'{formataNumero(total_visualizacoes)}\nInterações', ha='center', va='center', fontsize=14, fontweight='bold')
    ax.text(0, 0, f'{crescimento(total_visualizacoes,anterior)}', ha='center', va='center', fontsize=14, fontweight='bold')
    ...
    interacoesIG_plot_path = fr"C:/Users/{path_Usuarios}/Documents/Repositórios/Imagens/TN/interacoesIG.png"
    plt.savefig(interacoesIG_plot_path, bbox_inches="tight")

    return interacoesIG_plot_path

def grafico_barras_composto2(porcentagens):
    """
    Gera um gráfico de barras composto horizontal com as categorias 
    Publicações, Reels, Stories e Vídeos, na ordem inversa, com barras arredondadas.
    ...
    # Adiciona título e ajusta layout
    ax.set_title('Interações por Tipo de Conteúdo')
    plt.tight_layout()
    grafico_barras_composto2_plot_path = fr"C:/Users/{path_Usuarios}/Documents/Repositórios/Imagens/TN/grafico_barras_composto2.png"
    plt.savefig(grafico_barras_composto2_plot_path, bbox_inches="tight")

    return grafico_barras_composto2_plot_path
...
def seguidorFB():
    ...
    # Leitura dos dados de um único arquivo
    seguidoresIG = pd.read_csv(
        fr'C:\Users\{path_aliss}\Documents\Repositórios\csv\TNsemanal\SeguidoresFB.csv',
        skiprows=2, encoding='utf-16', skip_blank_lines=True
    )
    seguidoresIG['Data'] = pd.to_datetime(seguidoresIG['Data'])
    ...
    # Barras para Semana Anterior
    bars1 = ax.bar(x - width/2, valores_anteriores, width, label='Semana Anterior', color='#c4a667', edgecolor='black')
    # Barras para Semana Atual
    ...
    for x_pos, anterior, recente in zip(x, datas_anteriores, datas_recente):
        # Data da semana anterior
        ax.text(x_pos - width/2, -max(valores_anteriores.max(), valores_recente.max()) * 0.05, anterior, 
                ha='center', va='center', fontsize=9, color='#c4a667')
        # Data da semana atual
        ax.text(x_pos + width/2, -max(valores_anteriores.max(), valores_recente.max()) * 0.05, recente, 
                ha='center', va='center', fontsize=9, color='#3b5998')

    # Configurações do gráfico
    ax.set_title('Comparativo Diário de Seguidores no Facebook', fontsize=14)
    ax.set_ylabel('Quantidade de Seguidores')
    ax.set_xlabel('')
    ax.set_xticks(x)
    ax.set_xticklabels([''] * len(x))  # Remove os rótulos padrão do eixo X
    ...
    plt.tight_layout()
    seguidoresFB_plot_path = fr"C:/Users/{path_Usuarios}/Documents/Repositórios/Imagens/TN/seguidoresFB.png"
    plt.savefig(seguidoresFB_plot_path, bbox_inches="tight")
    
    return seguidoresFB_plot_path
...
def visitaFB():
    ...
    # Leitura dos dados de um único arquivo
    visitasIG = pd.read_csv(
        fr'C:\Users\{path_aliss}\Documents\Repositórios\csv\TNsemanal\VisitasFB.csv',
        skiprows=2, encoding='utf-16', skip_blank_lines=True
    )
    visitasIG['Data'] = pd.to_datetime(visitasIG['Data'])
    ...
    # Configurações do gráfico
    ax.set_title('Comparativo Diário de Visitas no Facebook', fontsize=14)
    ax.set_ylabel('Quantidade de Visitas')
    ax.set_xlabel('')
    ax.set_xticks(x)
    ax.set_xticklabels([''] * len(x))  # Remove os rótulos padrão do eixo X
...
def alcanceeFB():
    ...
    # Leitura dos dados de um único arquivo
    alcanceIG = pd.read_csv(
        fr'C:\Users\{path_aliss}\Documents\Repositórios\csv\TNsemanal\AlcanceFB.csv',
        skiprows=2, encoding='utf-16', skip_blank_lines=True
    )
    alcanceIG['Data'] = pd.to_datetime(alcanceIG['Data'])
    ...
    # Barras para Semana Anterior
    bars1 = ax.bar(x - width/2, valores_anteriores, width, label='Semana Anterior', color='#5874af', edgecolor='black')
    # Barras para Semana Atual
    bars2 = ax.bar(x + width/2, valores_recente, width, label='Semana Atual', color='#3b5998', edgecolor='black')
    ...
    for x_pos, anterior, recente in zip(x, datas_anteriores, datas_recente):
        # Data da semana anterior
        ax.text(x_pos - width/2, -max(valores_anteriores.max(), valores_recente.max()) * 0.05, anterior, 
                ha='center', va='center', fontsize=9, color='#5874af')
        # Data da semana atual
        ax.text(x_pos + width/2, -max(valores_anteriores.max(), valores_recente.max()) * 0.05, recente, 
                ha='center', va='center', fontsize=9, color='#3b5998')

    # Configurações do gráfico
    ax.set_title('Comparativo Diário de Alcance no Facebook', fontsize=14)
    ax.set_ylabel('Quantidade de Alcance')
    ax.set_xlabel('')
    ax.set_xticks(x)
    ax.set_xticklabels([''] * len(x))  # Remove os rótulos padrão do eixo X
...
def visualizacoeesFB():
    # Define o caminho para o arquivo
    path_aliss = "aliss"  # Substitua pelo seu usuário

    # Leitura dos dados de um único arquivo
    visualizacoesIG = pd.read_csv(
        fr'C:\Users\{path_aliss}\Documents\Repositórios\csv\TNsemanal\VisualizacoesFB.csv',
        skiprows=2, encoding='utf-16', skip_blank_lines=True
    )
    visualizacoesIG['Data'] = pd.to_datetime(visualizacoesIG['Data'])
    ...
    # Barras para Semana Anterior
    bars1 = ax.bar(x - width/2, valores_anteriores, width, label='Semana Anterior', color='#054f77', edgecolor='black')
    # Barras para Semana Atual
    bars2 = ax.bar(x + width/2, valores_recente, width, label='Semana Atual', color='#3b5998', edgecolor='black')
    ...
    for x_pos, anterior, recente in zip(x, datas_anteriores, datas_recente):
        # Data da semana anterior
        ax.text(x_pos - width/2, -max(valores_anteriores.max(), valores_recente.max()) * 0.05, anterior, 
                ha='center', va='center', fontsize=9, color='#054f77')
        # Data da semana atual
        ax.text(x_pos + width/2, -max(valores_anteriores.max(), valores_recente.max()) * 0.05, recente, 
                ha='center', va='center', fontsize=9, color='#3b5998')

    # Configurações do gráfico
    ax.set_title('Comparativo Diário de Visualizações no Facebook', fontsize=14)
    ax.set_ylabel('Quantidade de Visualizações')
    ax.set_xlabel('')
    ax.set_xticks(x)
    ax.set_xticklabels([''] * len(x))  # Remove os rótulos padrão do eixo X
    ...
    plt.tight_layout()
    visualizacoesFB_plot_path = fr"C:/Users/{path_Usuarios}/Documents/Repositórios/Imagens/TN/visualizacoesFB.png"
    plt.savefig(visualizacoesFB_plot_path, bbox_inches="tight")
    
    return visualizacoesFB_plot_path

def grafico_cidadesFB(cidades, porcentagens):
    """
    Gera um gráfico de barras simples para cidades e suas porcentagens, com as barras invertidas.
    ...
    fig, ax = plt.subplots(figsize=(12, 6))
    y = np.arange(len(cidades))  # Posição das cidades no eixo Y
    cor = '#3b5998'  # Cor rosa do Instagram
    ...
    cidadesFB_plot_path = fr"C:/Users/{path_Usuarios}/Documents/Repositórios/Imagens/TN/cidadesFB.png"
    plt.savefig(cidadesFB_plot_path, bbox_inches="tight")
    
    return cidadesFB_plot_path

def grafico_faixa_etariaFB(faixas, porcentagens):
    """
    Gera um gráfico de barras simples para faixas etárias e suas porcentagens.
    ...
    FEFB_plot_path = fr"C:/Users/{path_Usuarios}/Documents/Repositórios/Imagens/TN/FEFB.png"
    plt.savefig(FEFB_plot_path, bbox_inches="tight")
    
    return FEFB_plot_path
